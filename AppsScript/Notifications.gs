function sendAdminNotification(text, inlineKeyboard) {
  ...
  recipients.forEach(function (recipient) {
    if (inlineKeyboard) {
      sendTelegramMessageWithInlineKeyboard(settings.AdminBotToken, recipient.telegram_id, text, inlineKeyboard);

      return;
    }

    sendTelegramMessage(settings.AdminBotToken, recipient.telegram_id, text);
  });

  return true;
}

function notifyAdminsAboutCancellation(appointment) {
  const settings = getSettings();
  ...
  const text =
    '❌ ' +
    getMessage(MESSAGE_KEYS.ADMIN_APPOINTMENT_CANCELLED) +
    '\n\n' +
    '👤 ' +
...
function notifyAdminsAboutCustomerConfirmation(appointment) {
  const settings = getSettings();
  ...
  let text = '<b>' + getMessage(MESSAGE_KEYS.PROVIDER_CUSTOMER_CONFIRMED_APPOINTMENT) + '</b>\n\n';

  text += '👤 ' + (customer ? customer.name : '-') + '\n';

  text += '📞 ' + (customer ? customer.phone : '-') + '\n';

  text += '📅 ' + formatDateTimeForDisplay(appointment.start_at) + '\n';
