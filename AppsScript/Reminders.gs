function send24hAppointmentReminders() {
  ...
  const appointments = getAppointmentsFor24hReminder();

  appointments.forEach(function (appointment) {
    appointment = syncAppointmentWithCalendar(appointment);
    ...
    const text = buildAppointmentReminderText(appointment);

    sendTelegramMessage(
      settings.ClientBotToken,
      customer.telegram_id,
      text,
      buildAppointmentConfirmInlineKeyboard(appointment.appointment_id)
    );

    markAppointmentReminder24hSent(appointment.appointment_id);
  });
}

function buildAppointmentConfirmInlineKeyboard(appointmentId) {
  return {
    inline_keyboard: [
      [
        {
          text: getMessage(MESSAGE_KEYS.CUSTOMER_CONFIRM_APPOINTMENT),
...
function buildAppointmentReminderText(appointment) {
  ...
  return (
    getMessage(MESSAGE_KEYS.APPOINTMENT_REMINDER_24H) +
    '\n\n' +
    '📅 ' +
...
function markAppointmentReminder24hSent(appointmentId) {
  updateAppointmentField(appointmentId, 'reminder_24h_sent_at', new Date());
}
...
function getAppointmentsFor24hReminder() {
  ...
  const tomorrowString = Utilities.formatDate(tomorrow, timezone, 'yyyy-MM-dd');
  ...
  for (let i = 1; i < rows.length; i++) {
    ...
    const appointmentDateString = normalizeDateForStorage(appointment.start_at);
