...
function backToAdminMenu(chatId, settings, menu) {
  resetAdminWizard(chatId);

  const trimmed = trimNavigationToMenu(chatId, menu);
...
function handleAdminResultBack(chatId, settings, session) {
  const adminBackMenu = session.admin_back_menu || '';

  if (!adminBackMenu) {
    return false;
  }

  setUserSessionValue(chatId, 'admin_back_menu', '');

  backToAdminMenu(chatId, settings, adminBackMenu);
...
function handleScheduleBack(chatId, settings, state) {
  ...
  if (state === ADMIN_STATES.WAITING_SCHEDULE_START_TIME || state === ADMIN_STATES.WAITING_SCHEDULE_END_TIME) {
...
function setPreviousMenu(chatId, menuName) {
  setUserSessionValue(chatId, 'previous_menu', menuName);
}
...
function processAdminBack(chatId, settings) {
  ...
  handleAdminBackButton(chatId, settings);
}
