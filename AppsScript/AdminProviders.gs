...
function showProvidersListAdmin(chatId, settings) {
  setUserSessionValue(chatId, 'admin_back_menu', ADMIN_MENUS.PROVIDERS);
  ...
  let text = '<b>' + getMessage(MESSAGE_KEYS.PROVIDERS_LIST_TITLE) + '</b>\n\n';

  providers.forEach(function (provider, index) {
    const location = findLocationById(provider.location_id);

    text += String(index + 1) + '. <b>' + provider.name + '</b>\n';

    text += '📍 ' + (location ? location.name : provider.location_id) + '\n';
