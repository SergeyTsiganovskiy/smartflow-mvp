...
function initializeAdminNavigation() {
  ...
  ADMIN_NAVIGATION_HANDLERS[ADMIN_MENUS.APPOINTMENTS] = sendAppointmentsMenu;

  ADMIN_NAVIGATION_HANDLERS[ADMIN_MENUS.CUSTOMERS] = sendCustomersMenu;

  ADMIN_NAVIGATION_HANDLERS[ADMIN_MENUS.CUSTOMER_CONFLICTS] = startCustomerConflicts;
  ...
  ADMIN_NAVIGATION_HANDLERS[ADMIN_MENUS.CONFIGURATION] = sendConfigurationMenu;
}

function openAdminMenu(chatId, settings, menu) {
  if (Object.keys(ADMIN_NAVIGATION_HANDLERS).length === 0) {
    initializeAdminNavigation();
  }

  const handler = ADMIN_NAVIGATION_HANDLERS[menu];
...
function handleAdminBackButton(chatId, settings) {
  ...
  openAdminMenu(chatId, settings, current.menu);
...
function sendAdminMainMenu(chatId, settings) {
  ...
  pushNavigation(chatId, ADMIN_MENUS.MAIN);
  ...
  sendTelegramMessage(settings.AdminBotToken, chatId, getMessage(MESSAGE_KEYS.ADMIN_MAIN_MENU), keyboard);
}

function sendSettingsMenu(chatId, settings) {
  const stack = getNavigationStack(chatId);
  ...
  navigateAdmin(chatId, ADMIN_MENUS.SETTINGS);
  ...
  sendTelegramMessage(settings.AdminBotToken, chatId, getMessage(MESSAGE_KEYS.ADMIN_SETTINGS), keyboard);
}
