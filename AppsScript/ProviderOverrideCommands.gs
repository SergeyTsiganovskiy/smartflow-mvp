function generateOverrideId() {
  return generateNextEntityId('ProviderScheduleOverrides', 'override_id', 'ovr');
}
...
function createShortDayOverride(chatId) {
  const session = getUserSession(chatId);
  ...
  return createProviderScheduleOverride({
    ...
    start_time: session.override_start_time,

    end_time: session.override_end_time,
