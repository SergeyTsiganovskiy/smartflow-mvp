function processRequestApproveOption(
  callbackQuery,
  action,
  requestId
) {
  ...
  addAuditLog(
    ...
    JSON.stringify({
      ...
      option: option,
      appointmentExists:
        appointmentExistsForRequest(requestId)
    })
  );
  ...
  if (appointmentExistsForRequest(requestId)) {
    sendTelegramMessage(
      ...
      adminChatId,
      getMessage(
        MESSAGE_KEYS.REQUEST_ALREADY_PROCESSED
      )
    );
  ...
  if (customer && customer.telegram_id) {
    const clientText =
      getMessage(
        MESSAGE_KEYS.REQUEST_APPROVED_CLIENT
      ) +
      '\n\n' +
      '📅 ' +
...
function processRequestReject(
  ...
  if (
    ...
    sendTelegramMessage(
      ...
      customer.telegram_id,
      getMessage(
        MESSAGE_KEYS.REQUEST_REJECTED_CLIENT
      )
    );
  }
}
