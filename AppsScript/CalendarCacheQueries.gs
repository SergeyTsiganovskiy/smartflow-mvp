function getCachedAppointmentsByDate(dateValue) {
  ...
  const result = getCalendarCache().filter(function (item) {
    const cacheDate = normalizeDateForStorage(item.cache_date);

    const status = String(item.status || '').toLowerCase();

    return cacheDate === targetDate && status === 'confirmed';
  });
...
function getCachedAppointmentsByProvider(providerId) {
  ...
  const result = getCalendarCache().filter(function (item) {
    ...
    return String(item.provider_id) === String(providerId) && status === 'confirmed' && new Date(item.start_at) >= now;
  });
...
function getCustomerVisitHistory(phone) {
  ...
  const result = getCalendarCache().filter(function (item) {
    ...
    return getPhoneSearchKey(item.phone) === phoneKey && status === 'confirmed';
  });
